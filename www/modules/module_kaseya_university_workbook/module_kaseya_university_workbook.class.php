...
*/
class module_kaseya_university_workbook extends EfrontModule {
		...
		if ($_SESSION['s_type'] == 'administrator') {
			...
			if (defined('G_BRANCH_URL') && G_BRANCH_URL) {
				try {
					...
					$currentUser = $_SESSION['s_login'];
					//$query = eF_executeNew("Select user_types_ID from users where login = 'KCTTim'");
					$query = eF_getTableData("users", "user_types_ID", "login='$currentUser'");
					
					foreach ($query as $value) {
						if ($value[user_types_ID] == 9) { //KCA Folder Access. Currently KCA User Type
							$assets_path = $root_path = $this -> moduleBaseDir.'assets/89632147KCA/';
						}
						elseif ($value[user_types_ID] == 10) { //KCT Folder Access. Currently KCT User Type
							$assets_path = $root_path = $this -> moduleBaseDir.'assets/9874563210KCT/';
						}
						elseif ($value[user_types_ID] == 1) { //KCAKCT Folder Access. Currently KaseyaScholar User Type
							$assets_path = $root_path = $this -> moduleBaseDir.'assets/153698745KCAKCT/';
						}
						elseif ($value[user_types_ID] == 15) { //KCAKCT Folder Access. Currently FIU User Type
							$assets_path = $root_path = $this -> moduleBaseDir.'assets/153698745KCAKCT/';
						}
						elseif ($value[user_types_ID] == 17) { //KCAKCT Folder Access. Currently Employee User Type
							$assets_path = $root_path = $this -> moduleBaseDir.'assets/153698745KCAKCT/';
						}						
						else { //Else clause access to a blank folder
							$assets_path = $root_path = $this -> moduleBaseDir.'assets/Empty/';
						}
					}
				}
			$files = array();
