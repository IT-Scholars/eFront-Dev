...
* - Modified email declaration, so it can detect emails that have a dot (.) in the first part (before the '@').
*/
function eF_checkParameter_vLab($parameter, $type, $correct = false)
{
    switch ($type) {
        case 'login':
            if (!preg_match("/^_*\w+(\w*[._-]*\w*)*$/", $parameter) || mb_strlen($parameter) > 100) { 
			//This means: begins with 0 or more '_', followed by at least 1 word character, followed by any combination of .,_,- and word characters.
                return false;
            }
            break;

       default:
	   		return eF_checkParameter($parameter, $type, $correct);
    }
    return $parameter;
