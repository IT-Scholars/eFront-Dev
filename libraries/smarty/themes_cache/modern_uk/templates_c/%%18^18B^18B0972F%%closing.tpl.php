...
smarty_core_load_plugins(array('plugins' => array(array('function', 'eF_template_printBlock', 'includes/closing.tpl', 46, false),array('modifier', 'basename', 'includes/closing.tpl', 130, false),array('modifier', 'replace', 'includes/closing.tpl', 130, false),)), $this); ?>
<script>
...
" onclick = "if (document.getElementById('reloadHidden') && document.getElementById('reloadHidden').value == '1')  {parent.frames[1].location = parent.frames[1].location};eF_js_showDivPopup();"/>
    </td></tr>
...
<?php if (isset ( $this->_tpl_vars['div_error'] )): ?>
	eF_js_showDivPopup(event, '<?php echo $this->_tpl_vars['div_error']; ?>
');
<?php endif; ?>
...
<?php if ($this->_tpl_vars['T_FACEBOOK_ACCOUNT_MERGE_POPUP']): ?>
	...
	eF_js_showDivPopup(event, '<?php echo @_FACEBOOKMERGEACCOUNT; ?>
', 2, 'facebook_login');
	<?php else: ?>
	eF_js_showDivPopup(event, '<?php echo @_FACEBOOKMERGEACCOUNT; ?>
', 0, 'facebook_login');
...
<?php if ($this->_tpl_vars['T_CONFIGURATION']['facebook_api_key']): ?>     	      
	<?php echo '        	
	  <script>
	    window.fbAsyncInit = function() {
	      FB.init({
	        appId      : "'; ?>
<?php echo $this->_tpl_vars['T_CONFIGURATION']['facebook_api_key']; ?>
<?php echo '",
	        status     : true, 
	        cookie     : true,
	        xfbml      : true,
	    ...
	    function facebook_logout() {
	    	 FB.getLoginStatus(function(response) {
	    		 if (response.status === \'connected\') { 
			    		...
			    		window.location = "'; ?>
<?php echo @G_SERVERNAME; ?>
<?php echo 'index.php?logout=true"; 
		        	});
	    		} else {
	    			window.location = "'; ?>
<?php echo @G_SERVERNAME; ?>
